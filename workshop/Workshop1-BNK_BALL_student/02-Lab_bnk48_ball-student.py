...
ball3_img = pg.image.load("source/BNK48/Bamboo_cc.png").convert_alpha()
ball3_img = pg.transform.scale(ball3_img, (120, 120))
ball3_rect = ball3_img.get_rect(center=(800, 400))

# Radius of the balls
ball1R = (ball1_rect.top - ball1_rect.bottom) / 2
ball2R = (ball2_rect.top - ball1_rect.bottom) / 2
ball3R = (ball3_rect.top - ball1_rect.bottom) / 2

# Center of the balls
ball1C = [(ball1_rect.left + ball1_rect.right) / 2, (ball1_rect.top + ball1_rect.bottom) / 2]
ball2C = [(ball2_rect.left + ball2_rect.right) / 2, (ball2_rect.top + ball2_rect.bottom) / 2]
ball3C = [(ball3_rect.left + ball3_rect.right) / 2, (ball3_rect.top + ball3_rect.bottom) / 2]


def hit(r1, r2, c1, c2):
    return (r1 + r2) ** 2 >= (c1[0] - c2[0]) ** 2 + (c1[1] - c2[1]) ** 2
...
while running:
    ...
    if running:
        ...
        text_rect = text_surface.get_rect()  # แปลง Surface เปน object
        text_rect.midtop = (width/2, height/2.5)  # ระบตแหนงของ text
        ...
        font2 = pg.font.Font(font_name, 100)
        text_surface2 = font2.render("6531332121", True, white)
        text_rect2 = text_surface2.get_rect()  # แปลง Surface เปน object
        text_rect2.midtop = (width/2, height/1.5)  # ระบตแหนงของ text
        ...
        # Radius of the balls
        ball1R = (ball1_rect.top - ball1_rect.bottom) / 2
        ball2R = (ball2_rect.top - ball2_rect.bottom) / 2
        ball3R = (ball3_rect.top - ball3_rect.bottom) / 2
